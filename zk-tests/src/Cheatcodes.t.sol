...
contract ZkCheatcodesTest is Test {
    ...
    function testFetchCode() public {
        ...
        vm.startBroadcast();
        console.log("fails right after this message here");
        Greeter deposit = new Greeter();
        vm.stopBroadcast();
        // // Prepare the low-level call to fetchCode(address)
        // (bool success2, bytes memory returnData) = address(vm).call(
        //     abi.encodeWithSignature("fetchCode(address)", address(greeter))
        // );

        // // Ensure the call succeeded
        // require(success2, "fetchCode call failed");

        // // Decode the returned bytecode
        // bytes memory fetchedCode = abi.decode(returnData, (bytes));

        // // Ensure that the fetched code is not empty
        // assertGt(fetchedCode.length, 0, "Fetched code should not be empty.");
        // vm.stopBroadcast();
        // Fetch the expected bytecode using vm.getCode
        bytes memory expectedCode = vm.getCode("Greeter.sol:Greeter");
        
        // Verify that the fetched code matches the expected bytecode
        //assertEq(fetchedCode, expectedCode, "The fetched bytecode should match the expected bytecode.");
    }
}
