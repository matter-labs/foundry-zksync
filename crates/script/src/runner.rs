...
impl ScriptRunner {
    ...
    fn search_optimal_gas_usage(
        ...
        if matches!(res.exit_reason, Some(return_ok!())) {
            ...
            let init_gas_limit = self.executor.env().tx.base.gas_limit;
            ...
            while (highest_gas_limit - lowest_gas_limit) > 1 {
                ...
                self.executor.env_mut().tx.base.gas_limit = mid_gas_limit;
            ...
            self.executor.env_mut().tx.base.gas_limit = init_gas_limit;
        }
        Ok(gas_used)
    }
}
