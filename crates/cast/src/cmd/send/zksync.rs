...
pub async fn run_zk_send(
    ...
    let tx_hash = send_zk_transaction(zk_provider, builder, eth_opts, zk_tx_opts, zk_code).await?;

    let provider = ProviderBuilder::<_, _, AnyNetwork>::default().connect_provider(provider);
