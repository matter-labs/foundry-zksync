...
impl ProjectCompiler {
    ...
    fn zksync_handle_output(
        ...
        for (artifact_id, artifact) in output.artifact_ids() {
            ...
            if is_target_file {
                if let Some(mls) = artifact.bytecode.as_ref().map(|bc| &bc.missing_libraries) {
        ...
        let missing_libs: Vec<ZkMissingLibrary> = missing_libs_unique
            ...
            .map(|ml| {
                ...
                ZkMissingLibrary {
                    ...
                    missing_libraries: art.bytecode.as_ref().map(|bc| &bc.missing_libraries).cloned().unwrap_or_default(),
                }
            })
            .collect();
