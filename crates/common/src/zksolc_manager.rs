...
impl ZkSolcManager {
    ...
    pub fn get_full_compiler(&self) -> String {
        format!("{}v{}", self.compiler, self.version)
    }
...
pub async fn fetch_solc_metadata() -> Result<ZkSolcMetadata> {
    let client = Client::new();
    let response = client
    ...
    let solc_metadata: ZkSolcMetadataResponseBody = response
        .json()
        .await
        .map_err(|e| Error::msg(format!("Failed to parse solc versions file: {e}")))?;

    solc_metadata.try_into().map_err(Into::into)
}

/// Metadata of the solc-bin comiler.
#[derive(serde::Deserialize, Clone, Debug)]
#[serde(rename_all = "camelCase")]
pub(crate) struct ZkSolcMetadataResponseBody {
...
impl TryFrom<ZkSolcMetadataResponseBody> for ZkSolcMetadata {
    type Error = semver::Error;
    
    fn try_from(value: ZkSolcMetadataResponseBody) -> Result<Self, Self::Error> {
        Ok(Self {
            latest: semver::Version::parse(&value.latest)?,
            min_version: semver::Version::parse(&value.min_version)?,
        })
    }
}
