...
impl InspectorStack {
    ...
    pub fn set_env(&mut self, env: &Env) {
        self.set_block(&env.evm_env.inner.block_env);
        self.set_gas_price(env.tx.base.gas_price);
    }
...
impl InspectorStackRefMut<'_> {
    ...
    fn adjust_evm_data_for_inner_context(&mut self, ecx: &mut ZkContext<&mut dyn DatabaseExt>) {
        let inner_context_data =
            self.inner_context_data.as_ref().expect("should be called in inner context");
        ecx.tx.base.caller = inner_context_data.original_origin;
    }

    fn do_call_end(
        &mut self,
        ecx: &mut ZkContext<&mut dyn DatabaseExt>,
        inputs: &CallInputs,
    ...
    fn transact_inner(
        ...
        ecx.block.basefee = 0;
        ecx.tx.base.chain_id = Some(ecx.cfg.chain_id);
        ecx.tx.base.caller = caller;
        ecx.tx.base.kind = kind;
        ecx.tx.base.data = input;
        ecx.tx.base.value = value;
        // Add 21000 to the gas limit to account for the base cost of transaction.
        ecx.tx.base.gas_limit = gas_limit + 21000;

        // If we haven't disabled gas limit checks, ensure that transaction gas limit will not
        // exceed block gas limit.
        if !ecx.cfg.disable_block_gas_limit {
            ecx.tx.base.gas_limit = std::cmp::min(ecx.tx.base.gas_limit, ecx.block.gas_limit);
        }
        ecx.tx.base.gas_price = 0;

        self.inner_context_data =
            Some(InnerContextData { original_origin: cached_env.tx.base.caller });
        ...
        let res = self.with_inspector(|inspector| {
            ...
            *env.tx = cached_env.tx;
            env.block.basefee = cached_env.evm_env.inner.block_env.basefee;
    ...
    fn top_level_frame_start(&mut self, ecx: &mut ZkContext<&mut dyn DatabaseExt>) {
        if self.enable_isolation {
...
impl Inspector<ZkContext<&mut dyn DatabaseExt>> for InspectorStackRefMut<'_> {
    ...
    fn step(&mut self, interpreter: &mut Interpreter, ecx: &mut ZkContext<&mut dyn DatabaseExt>) {
...
impl Inspector<ZkContext<&mut dyn DatabaseExt>> for InspectorStack {
    fn step(&mut self, interpreter: &mut Interpreter, ecx: &mut ZkContext<&mut dyn DatabaseExt>) {
    ...
    fn call(
        &mut self,
        context: &mut ZkContext<&mut dyn DatabaseExt>,
