...
pub struct Env {
    pub evm_env: ZKsyncEnv,
    pub tx: ZKsyncTx<TxEnv>,
}

/// Helper container type for [`EvmEnv`] and [`TxEnv`].
impl Env {
    pub fn default_with_spec_id(spec_id: ZkSpecId) -> Self {
        let mut cfg = CfgEnv::default();
        cfg.spec = spec_id;

        Self::from(cfg, BlockEnv::default(), ZKsyncTx::default())
    }

    pub fn from(cfg: CfgEnv<ZkSpecId>, block: BlockEnv, tx: ZKsyncTx<TxEnv>) -> Self {
...
pub struct EnvMut<'a> {
    ...
    pub cfg: &'a mut CfgEnv<ZkSpecId>,
    pub tx: &'a mut ZKsyncTx<TxEnv>,
}
...
impl AsEnvMut for Env {
    fn as_env_mut(&mut self) -> EnvMut<'_> {
        EnvMut {
            block: &mut self.evm_env.inner.block_env,
            cfg: &mut self.evm_env.inner.cfg_env,
            tx: &mut self.tx,
        }
    }
}
...
impl Env {
    ...
    pub fn from(cfg: CfgEnv<SpecId>, block: BlockEnv, tx: TxEnv) -> Self {
    ...
    for ZkContext<DB, Journal<DB, JournalEntry>, C>
{
    ...
    fn as_db_env_and_journal(
        &mut self,
    ) -> (&mut Self::DB, &mut JournalInner<JournalEntry>, ZkEnvMut<'_>) {
        (
            &mut self.journaled_state.database,
            &mut self.journaled_state.inner,
            ZkEnvMut { block: &mut self.block, cfg: &mut self.cfg, tx: &mut self.tx },
        )
    }
}
