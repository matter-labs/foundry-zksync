...
impl Backend {
    ...
    pub(crate) fn initialize(&mut self, env: &Env) {
        ...
        self.set_spec_id(env.evm_env.inner.cfg_env.spec.into_eth_spec());
    ...
    pub fn replay_until(
        ...
        let full_block =
            fork.db.db.get_full_block(env.evm_env.inner.block_env.number.saturating_to::<u64>())?;
...
pub(crate) fn update_current_env_with_fork_env(current: &mut EnvMut<'_>, fork: Env) {
    *current.block = fork.evm_env.inner.block_env;
    *current.cfg = fork.evm_env.inner.cfg_env;
...
mod tests {
    ...
    async fn can_read_write_cache() {
        ...
        let meta = BlockchainDbMeta {
            chain: None,
            block_env: env.evm_env.inner.block_env,
