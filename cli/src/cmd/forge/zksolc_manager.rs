/// The `ZkSolcManager` module manages the downloading and setup of the `zksolc` Solidity compiler.
/// This module provides functionalities to interact with different versions of the zksolc compiler
/// as well as supporting different operating systems.
///
/// This module consists of several key components:
///
/// * `ZkSolcVersion`: This enum represents the available versions of the zksolc compiler. The `parse_version` function
///   accepts a string version and returns the corresponding `ZkSolcVersion` variant.
///
/// * `ZkSolcOS`: This enum represents the supported operating systems for the zksolc compiler.
///   `get_operating_system` function determines the current operating system and returns the corresponding `ZkSolcOS` variant.
///
/// * `ZkSolcManagerOpts`: This structure holds options for creating a `ZkSolcManagerBuilder`, which includes the version of the compiler to be used.
///
/// * `ZkSolcManagerBuilder`: This structure is used to construct a `ZkSolcManager`. It holds options like compilers path, version, compiler name,
///   and download URL. It has a `build` function which constructs a `ZkSolcManager` instance.
///
/// * `ZkSolcManager`: This structure manages a particular version of the zksolc compiler. It includes functions to get the full compiler name,
///   check if the compiler exists, setup the compilers directory, and download the compiler if necessary.
///
...
///
/// The `get_version` method is provided to get a string representation of the version associated with each enum variant.
///
/// This enumeration is used in the `ZkSolcManager` to specify the `zksolc` compiler version to be used for contract compilation.
...
impl ZkSolcOS {
    /// `get_compiler` provides the appropriate compiler string based on the current operating system.
    ///
    /// # Returns
    ///
    ...
    /// `get_download_uri` provides the appropriate URI for downloading the compiler binary based on the current operating system.
    ///
    /// # Returns
    ///
...
///
/// This builder pattern is used to encapsulate all the details needed to create a `ZkSolcManager`. The fields include:
///
/// * `compilers_path`: An optional `PathBuf` that denotes the directory where the compiler binaries are stored.
...
///
/// The builder provides a `new` function to initialize the structure with `ZkSolcManagerOpts`, and a `build` function
/// which constructs and returns a `ZkSolcManager` instance.
///
/// The `get_compiler` function is used to determine the appropriate compiler name based on the operating system.
///
/// The builder follows a fluent interface design, allowing the setting of properties in a chained manner.
/// The `build` function should be the final method invoked after all necessary properties have been set.
///
/// # Example
///
/// ```
...
impl ZkSolcManagerBuilder {
    ...
    ///
    /// This function takes a `ZkSolcManagerOpts` instance as a parameter, which contains the zkSync compiler version as a string.
    /// The builder's `version` field is initialized with this version. The `compilers_path` and `compiler` fields are initialized
    /// as `None` since they are optional and can be set later.
    ///
    /// The `download_url` field is initialized with the constant `ZKSOLC_DOWNLOAD_BASE_URL`, which is the base URL from where
    /// the compiler binary will be downloaded.
    ///
    /// This function should be used as the starting point for creating a `ZkSolcManager` instance using the builder pattern.
    ///
    /// # Parameters
    ///
    ...
    ///
    /// This function determines the current operating system using `get_operating_system`, and returns the corresponding
    /// compiler string based on the operating system. The compiler string is used to construct the filename for the
    /// `zksolc` compiler binary.
    ///
    /// # Returns
    ///
    /// Returns a `Result<String>` representing the compiler string based on the current operating system.
    ///
    /// # Errors
    ///
    ...
    ///
    /// This function finalizes the construction of a `ZkSolcManager` instance using the builder pattern. It validates the provided
    /// options, resolves the necessary details, and creates the manager instance.
    ///
    /// The function performs the following steps:
    /// 1. Obtains the home directory path and appends the `.zksync` directory to it, which represents the compilers directory.
    /// 2. Parses the provided version string and verifies if it matches one of the supported `ZkSolcVersion` variants.
    /// 3. Determines the appropriate compiler string based on the current operating system using the `get_compiler` function.
    /// 4. Constructs a `ZkSolcManager` instance with the resolved compilers directory, version, compiler, and download URL.
    ///
    /// # Returns
    ///
    /// A `Result` containing the constructed `ZkSolcManager` instance if the build process is successful, or an `Err` if any errors occur.
    ///
    /// # Errors
    ///
...
/// `ZkSolcManager` is a structure that manages a specific version of the `zksolc` Solidity compiler.
///
/// This structure encapsulates the functionality to interact with different versions of the `zksolc` compiler,
/// including downloading, setting up, and checking for the existence of the compiler. It provides an abstraction
/// for managing the `zksolc` compiler, making it easier for developers to use different versions without dealing
/// with the low-level details.
///
/// # Fields
///
/// * `compilers_path`: A `PathBuf` representing the directory where the compiler binaries are stored.
/// * `version`: A `ZkSolcVersion` representing the specific version of the `zksolc` compiler managed by this instance.
/// * `compiler`: A `String` representing the compiler name.
/// * `download_url`: A `Url` representing the base URL from which the `zksolc` compiler binary is downloaded.
///
/// # Example
///
/// ```
...
/// ```
///
/// The example above demonstrates the creation of a `ZkSolcManager` instance. It specifies the `compilers_path`
/// where the compiler binaries are stored, the `version` of the `zksolc` compiler, the `compiler` name, and the
/// `download_url` for the compiler binary.
///
/// # Implementation Details
///
/// The `ZkSolcManager` structure provides several methods to interact with the `zksolc` compiler:
///
/// * `get_full_compiler()`: Returns a `String` representing the full compiler name, including the version.
/// * `get_full_download_url()`: Returns a `Result<Url>` representing the full download URL for the compiler binary.
/// * `get_full_compiler_path()`: Returns a `PathBuf` representing the full path to the compiler binary.
...
/// * `download()`: Downloads the compiler binary if it doesn't already exist in the compilers directory.
///
...
impl ZkSolcManager {
    ...
    ///
    /// This function creates a new `ZkSolcManager` instance with the provided parameters. It initializes
    /// the manager with the compilers directory path, the version of the `zksolc` compiler, the compiler name,
    /// and the download URL for the compiler binary.
    ///
    /// # Parameters
    ///
    /// * `compilers_path`: A `PathBuf` representing the directory where the compiler binaries are stored.
    ...
    ///
    /// This function constructs and returns the full name of the `zksolc` compiler by combining the base compiler name
    /// with the specific version associated with the `ZkSolcManager` instance. The resulting string represents the
    /// complete name of the compiler, including the version.
    ///
    /// # Returns
    ///
    /// Returns a `String` representing the full name of the `zksolc` compiler, including the version.
    ///
    pub fn get_full_compiler(&self) -> String {
        return format!("{}{}", self.compiler, self.version.get_version());
    }

    /// Returns the full download URL for the `zksolc` compiler binary based on the current operating system.
    ///
    /// This function constructs the full download URL for the `zksolc` compiler binary by combining the base download URL
    /// with the appropriate download URI based on the current operating system. The resulting URL represents the location
    /// from which the compiler binary can be downloaded.
    ///
    /// # Returns
    ///
    ...
    ///
    /// This function checks if the `zksolc` compiler binary file exists in the compilers directory specified by `compilers_path`.
    /// and checks if it is a regular file and has executable permissions.
    ///
    /// # Returns
    ///
    /// Returns a `bool` indicating whether the compiler binary exists in the compilers directory and has executable permissions.
    ...
    ///
    /// This function checks if the compilers directory specified by `compilers_path` exists. If the directory doesn't exist,
    /// it creates the directory and any necessary parent directories using `fs::create_dir_all`.
    ///
    /// # Returns
    ///
    /// Returns a `Result<()>` indicating success if the compilers directory exists or is successfully created.
    ///
    /// # Errors
    ///
    /// This function can return an `Err` if an error occurs during the creation of the compilers directory, such as:
    ...
    /// Downloads the `zksolc` compiler binary if it doesn't already exist in the compilers directory.
    ///
    /// This function downloads the `zksolc` compiler binary from the specified download URL if it doesn't already exist
    /// in the compilers directory. It performs the following steps:
    /// 1. Checks if the compiler binary already exists in the compilers directory using the `exists` function.
    /// 2. If the binary exists, the function returns early without performing any download.
    /// 3. If the binary doesn't exist, it sends a HTTP GET request to the download URL to retrieve the binary.
    /// 4. If the download is successful, it creates the output file in the compilers directory and writes the binary data to it.
    /// 5. Finally, it sets the appropriate permissions for the downloaded compiler binary.
    ///
    /// # Returns
    ///
    /// Returns a `Result<()>` indicating success if the download and setup process completes without any errors.
    ///
    /// # Errors
    ///
    /// This function can return an `Err` if any errors occur during the download or setup process, including:
